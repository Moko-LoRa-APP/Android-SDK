...
public class  WriteFilterAdvRawData extends OrderTask {
    public byte[] data;
    private ArrayList<byte[]> mSplitRawDatas;
    private int index;
    private int size;

    public WriteFilterAdvRawData(ArrayList<String> filterRawDatas) {
        super(OrderType.CHARACTERISTIC, OrderEnum.WRITE_FILTER_RAW_DATA, OrderTask.RESPONSE_TYPE_WRITE_NO_RESPONSE);
    ...
    public byte[] assemble() {
        if (mSplitRawDatas == null || mSplitRawDatas.size() == 0) {
            data = new byte[5];
            data[0] = (byte) MokoConstants.HEADER_SEND;
